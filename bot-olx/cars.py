...
if __name__ == '__main__':
    system('clear')
    url_car = input('URL: ')
    # url_car = 'https://df.olx.com.br/distrito-federal-e-regiao/autos-e-pecas/carros-vans-e-utilitarios/saveiro-trendline-1-6-899588065'
    code = input('Código: ')
...
#    chrome.click_input_selector('#List3 > div:nth-child(1) > div > button.display__remove-btn')

    images = listdir(f'images')
    name = name.replace('/', ' ')
    print(name)
    pasta = f'{argv[1]}/{name} {year} {color}'
    mkdir(f'/home/vinicius/Downloads/Carros/Conta {pasta}')
    for image in images:
        chrome.add_image('#group-image-container > div.ads-forms__image-drag-and-drop > div.image-container__box > input', f'/home/vinicius/bot/images/{image}')
        move(f'images/{image}', f'/home/vinicius/Downloads/Carros/Conta {pasta}')
